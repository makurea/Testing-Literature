...
```java
// Куча — область памяти для хранения объектов и массивов
Object obj = new Object(); // хранится в куче
int[] array = new int[100]; // массив тоже в куче
```

- 💾 Используется для хранения **объектов**, **массивов**, **анонимных классов**.
- 🌐 Общая для всех потоков (shared memory).
- 🧹 Управляется **Garbage Collector'ом** — автоматическая очистка неиспользуемых объектов.
- ⚙️ Организована по поколениям:
  - **Young Generation** — новые объекты.
  - **Old Generation (Tenured)** — долго живущие объекты.
  - **Permanent Generation / Metaspace** — метаданные классов (в Java 8+ — Metaspace).
- 📏 Размер кучи можно настраивать:
  ```bash
  -Xms512m // начальный размер
  -Xmx2048m // максимальный размер
  ```
- 🔗 Объекты живут в куче, пока на них есть ссылки. Когда ссылки исчезают — объект становится кандидатом на удаление.

#### 📌 Stack (Стек)

```java
public void exampleMethod() {
    int x = 10; // переменная x хранится в стеке
    String s = "Hello"; // ссылка s в стеке, объект в куче
}
```

- 🧮 Используется для хранения **локальных переменных**, **параметров методов**, **ссылок на объекты**.
- 🔄 Каждый поток имеет **свой стек** — изолированная область памяти.
- 🧱 Организован как **стек вызовов**:
  - Каждый вызов метода — это **stack frame**.
  - При возврате из метода — фрейм удаляется.
- ⚡ Быстрый доступ к данным — стек работает по принципу **LIFO (Last In, First Out)**.
- 📉 Размер ограничен (например, 1MB–2MB по умолчанию), можно настроить:
  ```bash
  -Xss512k // размер стека на поток
  ```
- 🧠 Хранит:
  - Примитивные типы (`int`, `double`, `boolean` и т.д.)
  - Ссылки на объекты, которые находятся в куче.

#### 📝 Отличия Heap и Stack

| Характеристика        | Heap (Куча)                          | Stack (Стек)                          |
|----------------------|--------------------------------------|---------------------------------------|
| Тип данных           | Объекты, массивы                     | Примитивы, ссылки, вызовы методов     |
| Время жизни          | До тех пор, пока есть ссылки         | Пока выполняется метод                |
| Доступ               | Медленный, требует GC                | Быстрый, автоматический               |
| Общность             | Общая для всех потоков               | Уникальна для каждого потока          |
| Управление памятью   | Сборщик мусора                       | Автоматическое удаление фреймов       |
| Ошибки               | `OutOfMemoryError`                   | `StackOverflowError`                  |
| Настройка            | `-Xms`, `-Xmx`                       | `-Xss`                                |

#### ⚠️ Типичные ошибки

```java
// StackOverflowError — слишком глубокая рекурсия
public void recurse() {
    recurse(); // бесконечный вызов
}

// OutOfMemoryError — переполнение кучи
List<int[]> list = new ArrayList<>();
while (true) {
    list.add(new int[1000000]); // бесконечное выделение памяти
}
```

#### 📚 Полезные параметры JVM

```bash
-Xms<size>   // начальный размер кучи
-Xmx<size>   // максимальный размер кучи
-Xss<size>   // размер стека на поток
-XX:+UseG1GC // выбор сборщика мусора (например, G1)
```

#### ✅ Резюме

- Используй **Heap** для хранения объектов, особенно если они живут долго.
- Используй **Stack** для временных данных, локальных переменных и вызовов методов.
- Следи за глубиной рекурсии и объёмом создаваемых объектов.
- Настраивай параметры JVM для оптимизации производительности.
...
[🔄 К содержанию - главы](https://www.google.com/search?q=%23%D1%80%D0%B5%D1%84%D0%BB%D0%B5%D0%BA%D1%81%D0%B8%D1%8F-%D0%B8-%D0%B0%D0%BD%D0%BD%D0%BE%D1%82%D0%B0%D1%86%D0%B8%D0%B8-2)
[▶️ К содержанию](https://www.google.com/search?q=%23content)

-----

**Многопоточность (Multithreading)** в Java — это способность программы выполнять несколько задач (потоков) одновременно. Каждый поток — это независимый путь выполнения, который может работать параллельно с другими потоками, что делает программу более отзывчивой и эффективной.

### `Thread`

**`Thread`** — это основной класс для работы с потоками в Java. Каждый экземпляр класса `Thread` представляет собой отдельный поток выполнения.
...
**Недостатки этого подхода:**
  ...
  * Java не поддерживает множественное наследование, поэтому ваш класс не сможет наследовать другой класс.
  * Лучше использовать композицию, а не наследование, чтобы отделить задачу от самого механизма потока.

-----

### `Runnable`

**`Runnable`** — это интерфейс, который представляет собой задачу, которую можно выполнить в потоке. Интерфейс `Runnable` имеет только один метод — `run()`.
...
public class MyRunnable implements Runnable {
    @Override
    public void run() {
        System.out.println("Задача MyRunnable выполняется.");
    }
...
**Преимущества этого подхода:**
  ...
  * Ваш класс может наследовать другой класс, так как он реализует интерфейс, а не наследует класс.
  * Обеспечивает лучшую гибкость и разделение ответственности, поскольку вы отделяете код задачи (`Runnable`) от кода, который управляет потоком (`Thread`).
...
| `run()` | Содержит код, который будет выполняться в потоке. **Не вызывайте его напрямую\!** |
| `sleep(long millis)` | Приостанавливает выполнение текущего потока на указанное время в миллисекундах. |
| `join()` | Приостанавливает выполнение текущего потока до тех пор, пока поток, на котором вызван `join()`, не завершится. |
| `interrupt()` | Прерывает поток. Вызывает `InterruptedException`, если поток находится в состоянии ожидания (например, `sleep`, `join`). |

#### Сравнение `Thread` и `Runnable`

| Особенность | `Thread` | `Runnable` |
| :--- | :--- | :--- |
| **Реализация** | Наследование класса `Thread`. | Реализация интерфейса `Runnable`. |
| **Гибкость** | Ограничена, так как Java не поддерживает множественное наследование. | Высокая, так как класс может наследовать другой класс и реализовывать интерфейс. |
| **Разделение ответственности** | Не разделяет задачу и сам поток. | Четко разделяет задачу (`Runnable`) от механизма потока (`Thread`). |
| **Рекомендуется** | Реже используется. | **Рекомендуемый подход** для создания многопоточных приложений. |
...
`synchronized` — это ключевое слово в Java, которое обеспечивает **потокобезопасность**, предотвращая одновременный доступ нескольких потоков к одному и тому же блоку кода или методу. Это достигается за счет использования **встроенного монитора (intrinsic lock)** или **мьютекса (mutex)**.

Когда поток входит в `synchronized` метод или блок, он автоматически получает блокировку на объекте-мониторе. Другие потоки, которые пытаются получить ту же блокировку, будут заблокированы до тех пор, пока первый поток не освободит ее, выйдя из `synchronized` блока.

**Использование `synchronized`:**

  * **На методе экземпляра:**
    ...
    Блокировка применяется к объекту `this`. Только один поток может одновременно вызывать `synchronized` методы на одном и том же экземпляре `Counter`.
  ...
  * **На статическом методе:**
    ...
    Блокировка применяется к объекту `Class`. Только один поток может одновременно вызывать `synchronized` статические методы этого класса.
...
Блокировка применяется к указанному объекту `lock`. Это позволяет обеспечить потокобезопасность только для определенной части кода, а не для всего метода, что повышает производительность.

#### `volatile` 

`volatile` — это ключевое слово, которое гарантирует **видимость изменений** переменной для всех потоков. Оно решает две проблемы многопоточности:

1.  **Видимость (Visibility):** Когда один поток изменяет значение `volatile` переменной, это изменение становится сразу видимым для всех других потоков. Это предотвращает ситуацию, когда потоки работают с устаревшими (кэшированными) значениями переменной.
2.  **Упорядочивание (Ordering):** `volatile` устанавливает **отношение "происходит раньше" (happens-before)**, гарантируя, что все операции, которые произошли до записи в `volatile` переменную, станут видимыми для другого потока, который читает эту переменную. Это предотвращает **реорганизацию инструкций** компилятором.
...
public class SharedFlag {
    ...
    public void stop() {
        running = false; // Запись в volatile переменную
    }

    public void run() {
        while (running) { // Чтение volatile переменной
...
Без `volatile`, поток в методе `run()` мог бы бесконечно читать кэшированное значение `running = true`, игнорируя изменение, сделанное в другом потоке методом `stop()`.

### Сравнение `synchronized` и `volatile`

| Особенность | `synchronized` | `volatile` |
| :--- | :--- | :--- |
| **Основная цель** | Обеспечение атомарности и потокобезопасности. | Обеспечение видимости изменений переменной. |
| **Тип данных** | Применяется к методам или блокам кода. | Применяется только к переменным. |
| **Блокировка** | Да, обеспечивает эксклюзивный доступ. | Нет, не блокирует потоки. |
| **Производительность** | Может вызывать накладные расходы из-за блокировок. | Более легковесное, чем `synchronized`. |
| **Атомарность** | Гарантирует атомарность операций (например, `count++`). | Не гарантирует атомарность для операций, состоящих из нескольких шагов (чтение-изменение-запись). |

**Когда использовать:**

  * Используйте `synchronized` для **защиты критических секций**, где нужно гарантировать, что сложные операции (например, инкремент) выполняются целиком, без прерываний.
  * Используйте `volatile` для **флагов состояния** или переменных, которые должны быть видимы для всех потоков, но не требуют атомарных операций.
...
**Рефлексия** (Reflection) в Java — это механизм, который позволяет программе
анализировать и изменять свое собственное поведение во время выполнения.
С помощью рефлексии можно получать информацию о классах, методах, полях
и конструкторах, а также вызывать методы или изменять значения полей,
даже если они объявлены как `private`.
 
Основные классы для работы с рефлексией:
- `java.lang.Class`: Представляет класс или интерфейс.
- `java.lang.reflect.Method`: Представляет метод класса.
- `java.lang.reflect.Field`: Представляет поле (переменную) класса.
- `java.lang.reflect.Constructor`: Представляет конструктор класса.
...
public class ReflectionExample {
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("\n--- Работа с полями ---");
        // Пример работы с полями (в данном случае, внутреннее поле класса String)
        try {
            Class<?> stringValueClass = Class.forName("java.lang.String");
            Field valueField = stringValueClass.getDeclaredField("value");
            valueField.setAccessible(true); // Отключаем проверку доступа для приватного поля

            String myString = "hello";
            // Получаем значение поля
            char[] value = (char[]) valueField.get(myString);
            System.out.println("Значение приватного поля 'value': " + new String(value));
        } catch (NoSuchFieldException e) {
            System.out.println("Поле 'value' не найдено. Это может быть связано с версией Java.");
        }
    }
}
```
...
**Аннотации** (Annotations) — это метаданные, которые можно добавлять
в исходный код Java. Они не влияют на выполнение программы напрямую,
но могут быть обработаны компилятором, средой разработки или инструментами
во время выполнения, часто с помощью рефлексии.

Стандартные аннотации:
- `@Override`: Указывает, что метод переопределяет метод суперкласса.
- `@Deprecated`: Помечает элемент, который устарел и не рекомендуется к использованию.
- `@SuppressWarnings`: Указывает компилятору игнорировать определенные предупреждения.
...
public class MyClass {
    ...
    private void someMethod() {
        // Этот метод не используется, но компилятор не выдаст предупреждение.
    }
}
```
...
Чтобы создать свою собственную аннотацию, используется ключевое слово `@interface`.
Аннотация может содержать элементы (методы), которые представляют собой ее параметры.

Основные мета-аннотации:
- `@Retention`: Указывает, как долго аннотация будет доступна.
- `RetentionPolicy.SOURCE`: Доступна только в исходном коде.
- `RetentionPolicy.CLASS`: Доступна в скомпилированном .class файле. По умолчанию.
- `RetentionPolicy.RUNTIME`: Доступна во время выполнения.
- `@Target`: Указывает, к каким элементам кода можно применять аннотацию (классы, методы, поля).
- `@Documented`: Указывает, что аннотация должна быть включена в Javadoc.
- `@Inherited`: Указывает, что аннотация будет наследоваться подклассами.
...
@Target({ElementType.METHOD, ElementType.FIELD, ElementType.TYPE}) // Можно применять к методам, полям, классам
@Documented // Включить в Javadoc
public @interface MyCustomAnnotation {
    String value() default "default value"; // Элемент с именем 'value'
    int count() default 1;                 // Элемент 'count' с типом int
}

@MyCustomAnnotation(value = "hello class", count = 10)
class AnnotatedClass {
    @MyCustomAnnotation(value = "hello field", count = 2)
    private String myField;

    @MyCustomAnnotation(value = "hello method", count = 5)
...
Рефлексия и аннотации широко используются в современных фреймворках и библиотеках для автоматической настройки и выполнения кода. Это позволяет разработчикам не писать много шаблонного кода.

Вот несколько примеров:
**Spring Framework** использует аннотации `@Autowired` и `@Component` для автоматического управления зависимостями (Dependency Injection). Spring сканирует классы с этими аннотациями и самостоятельно создаёт их экземпляры, а затем внедряет их туда, где они нужны.
**JUnit** — фреймворк для тестирования. Он использует аннотации `@Test` и `@BeforeEach`, чтобы находить и запускать тестовые методы, а также управлять их жизненным циклом.
**Hibernate** — ORM-фреймворк, который использует аннотации `@Entity` и `@Id` для сопоставления Java-объектов с таблицами в базе данных (Object-Relational Mapping).

Механизм работы прост: библиотека, например, Spring, сканирует classpath проекта. Затем она находит классы и методы, помеченные определёнными аннотациями, и с помощью рефлексии считывает из них метаданные. В зависимости от этих данных, библиотека выполняет нужные действия: создаёт объекты, вызывает методы или настраивает соединения.
