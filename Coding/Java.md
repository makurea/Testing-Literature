...
| Вид класса                | Описание                                                                                              | Пример                                                    |
|--------------------------|-----------------------------------------------------------------------------------------------------|-----------------------------------------------------------|
| **Обычный класс**         | Стандартный класс, который можно создавать, наследовать, использовать для описания объектов          | `public class Person { private String name; }`             |
| **Абстрактный класс**     | Класс, который нельзя создать напрямую (нельзя вызвать `new`), может содержать абстрактные методы (без реализации), предназначен для наследования | `public abstract class Animal { abstract void move(); }`  |
| **Вложенный класс**       | Класс, объявленный внутри другого класса. Может быть обычным или статическим                         | `class Outer { class Inner { } }`                          |
| **Статический вложенный класс** | Вложенный класс с ключевым словом `static`. Не связан с экземпляром внешнего класса, может использоваться отдельно | `class Outer { static class Nested { } }`                  |
| **Локальный класс**       | Класс, объявленный внутри метода. Область видимости — только в этом методе                          | `void method() { class Local { void hello() {} } }`        |
| **Анонимный класс**       | Безымянный класс, который создаётся для реализации интерфейса или наследования класса один раз прямо при создании объекта | `Runnable r = new Runnable() { public void run() {} };`    |
