...
## Моки и Стабы <a id="моки-и-стабы"></a>

**Что это такое?**
При тестировании программ важно проверять отдельные части программы без зависимости от других компонентов, например, базы данных или внешних сервисов. Чтобы тесты были удобнее и быстрее, используют **моки (Mocks)** и **стабы (Stubs)**. Они помогают заменить настоящие объекты тестовыми версиями.

**В чем разница?**

**Стаб (Stub)** — это простая замена настоящего объекта, которая просто возвращает заранее заданные данные. Он не проверяет, как именно с ним работают, а просто подставляет нужные значения.

**Когда используют стабы?**
- Когда нужно просто заменить данные и избежать работы с реальным сервисом.
- Когда важно получить предсказуемый результат.

*Пример: если тестируемая программа запрашивает список товаров у базы данных, стаб просто всегда будет возвращать один и тот же список без настоящего запроса.*

**Мок (Mock)** — это более "умная" подмена объекта, которая не только возвращает данные, но и фиксирует, какие запросы к ней делали. Он позволяет проверять, сколько раз его вызвали и какие аргументы передали.

**Когда используют моки?**
- Когда важно проверить, как программа взаимодействует с другим компонентом.
- Когда нужно убедиться, что метод вызывается нужное количество раз с правильными данными.

*Пример: если тестируемая программа должна отправлять уведомления пользователю, мок проверит, действительно ли было отправлено уведомление и с каким текстом.*

**Основные отличия**

| Что сравниваем | Стаб (Stub) | Мок (Mock) |
|---------------|------------|------------|
| Что делает | Возвращает заранее заданные данные | Проверяет, как с ним работают |
| Контролирует вызовы? | Нет | Да |
| Где применяется | Там, где нужны только данные | Там, где важно проверить взаимодействие |

**Когда что использовать?**
- **Стабы** лучше, если тестируемому коду просто нужны данные для работы.
- **Моки** лучше, если нужно проверить, как программа взаимодействует с внешним компонентом.

Моки и стабы помогают тестировать код быстрее и проще, без необходимости зависеть от реальных сервисов. Стабы дают предсказуемые данные, а моки помогают проверять, как программа работает с другими модулями. Выбор зависит от того, что именно нужно проверить в тесте.
