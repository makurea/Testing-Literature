...
1. **Компонентное (модульное) интеграционное тестирование**  
   Проверка взаимодействия между модулями внутри одного компонента.  
   *Пример: тестирование связи между классами в одном сервисе.*

2. **Межкомпонентное (сервисное) интеграционное тестирование**  
   Проверка взаимодействия между разными компонентами системы.  
   *Пример: тестирование API между сервисом авторизации и основным backend.*

3. **Системное интеграционное тестирование**  
   Проверка взаимодействия между различными системами (микросервисами, внешними API).  
   *Пример: интеграция платежного шлюза с основной системой.*

4. **Внешнее интеграционное тестирование**  
   Проверка взаимодействия с внешними системами (API, облачные сервисы, БД).  
   *Пример: работа с API Google Maps или отправка email через SMTP.*

5. **Энд-ту-энд (E2E) интеграционное тестирование**  
   Проверка всего потока данных от фронтенда до бэкенда и внешних систем.  
   *Пример: полный сценарий оформления заказа (UI → оплата → уведомления).*
...
**Основные компоненты тестового стенда**  

🔹 **Оборудование (Hardware)**  
- Серверы (физические или виртуальные).  
- Клиентские устройства (ПК, мобильные устройства, планшеты).  
- Сетевое оборудование (роутеры, коммутаторы, VPN, прокси).  
...
- Средства автоматизации тестирования (Selenium, JMeter, Postman).  

🔹 **Тестовые данные (Test Data)**  
- Реальные данные (обезличенные).  
- Синтетические данные (сгенерированные специально для тестов).  
- Тестовые учетные записи и роли пользователей.  

🔹 **Средства мониторинга и логирования**  
- Логи сервера и приложений (ELK Stack, Splunk, Graylog).  
- Мониторинг производительности (New Relic, Grafana, Prometheus).  
- Системы обнаружения ошибок (Sentry, Bugsnag).  

**Типы тестовых стендов**  

🔸 **Локальный (Local Environment)**  
- Разворачивается на машине тестировщика.  
- Подходит для первичных проверок, ручного и автоматизированного тестирования.  

🔸 **Интеграционный (Integration Environment)**  
- Объединяет несколько сервисов для проверки взаимодействия.  
- Используется для API-тестирования, интеграционного и функционального тестирования.  

🔸 **Предпродакшен (Staging/Pre-Production)**  
- Полностью повторяет продакшен.  
- Проверяются все сценарии, нагрузка и безопасность перед выпуском.  

🔸 **Производственная среда (Production Environment - Prod)**  
- Рабочая среда, где продукт доступен пользователям.  
- Может использоваться для A/B-тестирования и мониторинга.  

**Лучшие практики конфигурации тестовых стендов**  

✅ Стенды должны быть изолированы друг от друга.  
✅ Конфигурация должна быть задокументирована.  
✅ Использовать контейнеризацию (Docker, Kubernetes) для быстрого развертывания.  
✅ Автоматизировать создание и настройку стендов (Terraform, Ansible, Puppet).  
✅ Данные должны быть анонимизированы и соответствовать GDPR.  

Грамотно настроенный тестовый стенд — залог успешного тестирования! 🚀  
...
🔹 **Для Kubernetes**  
- `.yaml` файлы (`deployment.yaml`, `service.yaml`) – определяют деплой, сервисы, настройки окружения.  

🔹 **Для Ansible**  
- `.yml` файлы (`playbook.yml`) – содержат инструкции по настройке серверов.  

🔹 **Для Terraform**  
- `.tf` файлы (`main.tf`, `variables.tf`) – описывают инфраструктуру как код.  

🔹 **Для CI/CD (Jenkins, GitLab CI/CD)**  
- `Jenkinsfile` – определяет pipeline для развертывания.  
...
- **Назначение**: HTTP — это протокол, используемый для передачи гипертекстовых документов, таких как веб-страницы, между клиентом (например, браузером) и сервером.
- **Уровень модели OSI**: Работает на уровне прикладного слоя модели OSI.
- **Порт**: HTTP использует порт **80** для передачи данных по сети.
- **Особенности**: 
  - HTTP не шифрует данные, передаваемые между клиентом и сервером, что делает его уязвимым для различных видов атак, например, "man-in-the-middle" (MITM), когда злоумышленник может перехватывать или изменять передаваемые данные.
  - В HTTP отсутствует механизм аутентификации и целостности данных, что ограничивает его безопасность.
  
#### HTTPS (Hypertext Transfer Protocol Secure)
- **Назначение**: HTTPS — это защищенная версия HTTP, которая использует криптографические протоколы SSL/TLS для обеспечения безопасности передачи данных между клиентом и сервером.
- **Уровень модели OSI**: Работает на уровне прикладного слоя модели OSI, но включает дополнительную безопасность за счет SSL/TLS.
- **Порт**: HTTPS использует порт **443**, который отличается от порта 80, используемого HTTP.
- **Особенности**:
  - HTTPS защищает данные путем их шифрования, что предотвращает перехват и изменение данных во время их передачи. Это делает невозможным выполнение атак типа "Man-in-the-middle".
  - Также обеспечивается аутентификация сервера через SSL/TLS-сертификаты, что помогает удостовериться, что соединение устанавливается именно с тем сервером, с которым пользователь намеревался установить соединение.
  - HTTPS используется для защиты конфиденциальной информации, такой как пароли, данные кредитных карт и личные данные.

#### SSL/TLS Handshake (Процесс рукопожатия SSL(Secure Sockets Layer)/TLS(Transport Layer Security))
- **Назначение**: Процесс SSL/TLS Handshake используется для установления защищенного соединения между клиентом и сервером, обеспечивая безопасность данных при их передаче.
- **Этапы SSL/TLS Handshake**:
  1. **Client Hello**:
     - Клиент (например, веб-браузер) отправляет серверу сообщение "Client Hello", которое содержит информацию о поддерживаемых криптографических алгоритмах (шифры), версиях SSL/TLS, а также случайное значение для безопасности.
  
  2. **Server Hello**:
     - Сервер отвечает сообщением "Server Hello", в котором указывает выбранные параметры безопасности (шифр, версия протокола SSL/TLS и случайное значение). Также сервер отправляет свой SSL/TLS-сертификат, который содержит открытый ключ.
  
  3. **Аутентификация и обмен ключами**:
     - Сервер отправляет свой **сертификат**, который должен быть проверен клиентом на подлинность (посредством доверенного центра сертификации).
     - Клиент проверяет сертификат сервера на соответствие с доверенным центром сертификации. Если сертификат действителен, клиент продолжает процесс.
     - В случае использования **диффи-Хеллмана** или **ECDHE** (криптографических методов для обмена ключами), происходит обмен секретным ключом, который будет использоваться для шифрования данных в ходе сессии.
  
  4. **Генерация общего секретного ключа**:
     - Используя информацию из этапов "Client Hello" и "Server Hello", оба участника рукопожатия (клиент и сервер) генерируют общий секретный ключ для шифрования данных.
  
  5. **Client Finished**:
     - Клиент отправляет сообщение, которое зашифровано с использованием сессионного ключа и содержит хеш-сумму всего рукопожатия (который проверяет целостность передаваемых данных).
  
  6. **Server Finished**:
     - Сервер отвечает аналогичным образом, подтверждая завершение процесса рукопожатия.
  
  7. **Установление зашифрованного канала**:
     - После успешного завершения рукопожатия клиент и сервер начинают обмениваться данными через зашифрованный канал, используя общий секретный ключ.

- **Основные задачи SSL/TLS Handshake**:
  - **Аутентификация**: Проверка подлинности сервера (и иногда клиента).
  - **Шифрование**: Установка безопасного канала для передачи данных.
  - **Целостность данных**: Обеспечение того, чтобы данные не были изменены во время передачи.
...
| **Видимость данных**| Данные в URL (`?ключ=значение`)        | Данные в теле запроса (не видны в URL)  |
| **Кэширование**     | Да (браузер может кэшировать)          | Нет                                     |
| **Безопасность**    | Менее безопасен (данные в URL)         | Безопаснее (данные скрыты)              |
| **Длина данных**    | Ограничено длиной URL (~2048 символов) | Нет жестких ограничений                 |
| **Идемпотентность** | Да (повторный запрос не меняет данные) | Нет (может изменять данные)             |
| **Использование**   | Поиск, фильтрация, загрузка страниц    | Формы, загрузка файлов, API             |
