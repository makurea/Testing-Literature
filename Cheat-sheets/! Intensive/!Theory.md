...
#### Основные виды стендов

##### 1. **Development (Dev)**
- Используется разработчиками для написания и отладки кода.
- Может содержать нестабильные версии приложения.
- Часто обновляется и не подходит для полноценных тестов.

##### 2. **Testing (Test)**
- Основное окружение для тестирования.
- Здесь тестировщики проводят функциональное, регрессионное и интеграционное тестирование.
- Данные в базе могут быть искусственно созданными.

##### 3. **Staging (Stage, Pre-prod)**
- Максимально приближено к продакшену.
- Используется для финального тестирования перед релизом.
- Может работать с копией боевых данных или синтетическими данными.

##### 4. **Production (Prod)**
- Рабочее окружение, доступное конечным пользователям.
- Здесь запрещено проводить тестирование, за исключением мониторинга и A/B-тестов.
- Любые изменения проходят строгий процесс деплоя.

#### Дополнительные стенды
Кроме основных, могут существовать и другие окружения, такие как:
- **UAT (User Acceptance Testing)** – используется для приемочного тестирования.
- **Performance (Perf)** – для нагрузочного тестирования.
- **Sandbox** – отдельная тестовая площадка для экспериментов.

#### Почему важно разделять стенды?
- Обеспечивает стабильность продакшена.
- Позволяет тестировать новые версии перед выпуском.
- Исключает влияние тестовых данных на реальных пользователей.

Разделение окружений помогает команде разработки и тестирования эффективно управлять качеством продукта, снижая риски на этапе релиза.
...
- **REST API(Representational State Transfer API)**: использует архитектурный стиль REST и передает данные в формате JSON или XML. RESTful API основано на принципах работы с ресурсами через HTTP-методы (GET, POST, PUT, DELETE).
- **SOAP API(Simple Object Access Protocol API)**: использует протокол SOAP и передает данные в формате XML. SOAP API требует более строгой структуры и может работать через различные протоколы, такие как HTTP, SMTP, TCP.
...
- **gRPC API(Google Remote Procedure Call API)**: использует протокол HTTP/2 и сериализацию данных через Protocol Buffers (protobuf), что позволяет достигать высокой производительности и низкой задержки.
...
- **Bearer Token** — простой тип токена, который передаётся в заголовке запроса. Он не содержит дополнительной информации, а лишь служит ключом доступа.

- **JWT (JSON Web Token)** — токен, который включает в себя зашифрованную информацию о пользователе или сессии. Обычно состоит из заголовка, полезной нагрузки и цифровой подписи.

- **Refresh Token** — используется для получения нового access-токена, когда тот истёк. Refresh-токен действует дольше и обеспечивает безопасность при длительной сессии.

#### 📥 Где передаются токены

Наиболее часто токены передаются в HTTP-заголовке запроса. Также возможна передача через cookie или параметры запроса, однако это менее безопасно и используется редко. Для защиты от атак рекомендуется использовать защищённые каналы и специальные флаги безопасности при хранении токенов.

#### 🧩 Преимущества токенов

- Отсутствие состояния на сервере (stateless)
- Удобная работа с распределёнными системами
- Гибкое управление правами доступа
- Подходит для SPA, мобильных и десктопных приложений

#### ⚠️ Риски и безопасность

Использование токенов требует соблюдения правил безопасности:
- Передавать токены только по HTTPS
- Использовать короткий срок действия для access-токенов
- Хранить токены безопасно (например, с флагом HttpOnly)
- Реализовать механизм обновления и отзыва токенов
