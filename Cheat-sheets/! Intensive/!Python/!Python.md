...
Декоратор `@property` позволяет обращаться к методам как к атрибутам, обеспечивая **инкапсуляцию** и контроль доступа к данным.

Позволяет определить:
- геттер (`@property`)
- сеттер (`@<property_name>.setter`)
- делетер (`@<property_name>.deleter`)

Полезен, когда нужно управлять доступом к полям класса без изменения интерфейса.

[🔼 К содержанию](#content)

---

## 🟠Исключения и декораторы <a id="исключения-и-декораторы"></a>
