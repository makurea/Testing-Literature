...
#### Apache Kafka: высокопроизводительный распределённый брокер сообщений

##### Основные характеристики

**Apache Kafka** — это распределённая платформа для обработки потоковых данных в реальном времени, разработанная LinkedIn и переданная в open source.

#### Ключевые особенности:
- **Высокая пропускная способность** (миллионы сообщений в секунду)
- **Низкая задержка** (менее 10 мс)
- **Горизонтальная масштабируемость**
- **Встроенная отказоустойчивость** через репликацию
- **Долговременное хранение** сообщений (по умолчанию 7 дней)

#### Архитектурные компоненты

#### Топики (Topics)
Логические каналы для категоризации сообщений. Например: `user_actions`, `payment_events`.

#### Партиции (Partitions)
Топики разделяются на партиции для параллельной обработки. Каждая партиция:
- Упорядочивает сообщения
- Реплицируется на несколько брокеров
- Обслуживается одним consumer'ом в группе

#### Репликация
Каждая партиция имеет:
- **Лидера** (обслуживает запросы)
- **Фолловеров** (синхронизированные копии)

#### Производители и потребители

#### Producers
Отправляют сообщения в топики, определяя:
- Целевой топик
- Ключ сообщения (опционально)
- Значение (payload)
- Таймстамп

#### Consumers
Читают сообщения через:
- **Consumer Groups** (параллельная обработка)
- **Offset management** (контроль позиции чтения)
- **Exactly-once семантику** (гарантия однократной обработки)

#### Использование в современных системах

#### Основные сценарии:
1. **Потоковая обработка данных** (ETL pipelines)
2. **Микросервисная коммуникация**
3. **Event sourcing** и CQRS
4. **Метрики и мониторинг**
5. **Хранение журналов событий**

#### Интеграция с экосистемой:
- **Kafka Connect** (интеграция с внешними системами)
- **Kafka Streams** (библиотека для потоковой обработки)
- **ksqlDB** (потоковый SQL-движок)

#### Преимущества перед традиционными очередями

| Характеристика       | Kafka | Традиционные очереди |
|----------------------|-------|----------------------|
| Производительность   | ⚡ Очень высокая | � Средняя |
| Масштабируемость     | 🔝 Горизонтальная | ⇳ Вертикальная |
| Хранение сообщений   | 🗄️ Долговременное | ⏳ Кратковременное |
| Гарантии доставки    | ✅ Exactly-once | ⚠️ At-least-once |
...
- Необходима потоковая обработка
- Важна горизонтальная масштабируемость

**Альтернативы для простых сценариев:**
- RabbitMQ (для RPC и рабочих очередей)
- NATS (для pub/sub с низкой задержкой)
- AWS SQS (managed очередь для облачных приложений)

> **Pro tip:** Для новых проектов рассмотрите Kafka с Kraft mode (без Zookeeper) — упрощённая архитектура с Kafka 3.0+
