...
- **Окружения (Environment Variables)** – позволяют задавать значения для конкретного окружения (например, тестовое, продакшен).
- **Коллекционные переменные** – принадлежат одной коллекции и используются внутри неё.
- **Локальные переменные** – существуют только во время выполнения запроса.
- **Файловые переменные** – используются в тестах при загрузке файлов.
