...
- **Строки**
  ...
  - [Задача 5.1: 🔥Развернуть строку (использование рекурсии и лямбда-выражения)](#task5l)  
...
public class Main {
    public static void main(String[] args) {
        String input = "привет";
        Function<String, String> reverse = s -> s.isEmpty() ? s : s.charAt(s.length() - 1) + reverse.apply(s.substring(0, s.length() - 1));
