...
| `Comparator<T>` | Сортировка списков для сравнения с эталоном | Отсортировать список сотрудников по полю `salary` в порядке убывания, чтобы сравнить с отсортированным эталонным списком |
...
| `Consumer<T>` | Выполнение действий без возврата результата | В цикле `forEach()` распечатать логи для каждого элемента списка или заполнить все поля формы тестовыми данными |
...
| `RestAssured` (интерфейс RequestSpecification) | Настройка и отправка API-запросов | Сконфигурировать базовый URI, заголовки и авторизацию через интерфейс `RequestSpecification`, а затем переиспользовать этот объект в разных тестах |
| `Response` | Работа с ответами API | Получить ответ от сервера через интерфейс `Response`, извлечь из него статус-код, тело и заголовки для последующих проверок |
