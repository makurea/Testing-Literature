...
**Bitbucket** — это веб-хостинг для репозиториев Git и Mercurial (теперь только Git) от компании Atlassian. Как и Jira с Confluence, входит в экосистему Atlassian и тесно интегрируется с ними. Используется командами разработки и тестирования для хранения кода, code review, CI/CD (Bitbucket Pipelines) и управления доступом.

#### 📦 Версии

| Версия | Описание | Для кого |
|--------|----------|----------|
| **Bitbucket Cloud** | Облачное решение (SaaS) | Малые и средние команды, не хотят заморачиваться с сервером |
| **Bitbucket Data Center** | On-premise (у себя на серверах) | Крупные компании с высокими требованиями к безопасности и отказоустойчивости |
| **Bitbucket Server** | On-premise, устаревшая (заменён на Data Center) | — |

#### 📐 Основные понятия

| Понятие | Описание |
|---------|----------|
| **Repository (Repo)** | Хранилище кода проекта |
| **Workspace (проект)** | Группа репозиториев с общими настройками и доступом |
| **Branch** | Ветка для разработки (main/master, develop, feature/*, bugfix/*) |
| **Pull Request (PR)** | Запрос на вливание изменений из одной ветки в другую (аналог Merge Request в GitLab) |
| **Approval** | Подтверждение PR другим разработчиком / тестировщиком |
| **Build Status** | Статус проверки CI/CD (проходит/не проходит) — отображается в PR |
| **Fork** | Копия репозитория в другой workspace (для Open Source или внешних контрибьюторов) |
| **Webhook** | HTTP-уведомление о событиях (push, PR, коммиты) для интеграции с Jenkins, GitHub Actions и др. |
| **Bitbucket Pipelines** | Встроенный CI/CD на базе Docker (только Cloud) |
| **SSH / App Password** | Способы аутентификации при работе с репозиторием через Git |

#### 🔧 Что делает тестировщик в Bitbucket

| Действие | Описание | Как делать |
|----------|----------|------------|
| **Клонировать репозиторий** | Скачать код на локальную машину для написания или запуска тестов | `git clone https://bitbucket.org/workspace/repo.git` |
| **Создать ветку** | Ветка для тестов, фикса бага или новой фичи | `git checkout -b feature/test-branch` |
| **Запустить тесты локально** | Убедиться, что тесты проходят перед пуллом | `mvn test` / `./gradlew test` |
| **Запушить изменения** | Отправить ветку с тестами в удалённый репозиторий | `git push origin feature/test-branch` |
| **Создать Pull Request** | Запросить вливание тестов в основную ветку | Bitbucket UI → Create Pull Request |
| **Проверить CI статус** | Убедиться, что пайплайн/тесты прошли | В PR смотреть статус билда ✅ / ❌ |
| **Code Review тестов** | Проверить, что тесты других написаны корректно | Открыть PR → вкладка Diff → комментировать строки |
| **Одобрить PR** | Подтвердить, что изменения готовы к влитию | PR → Approve (или +1) |
| **Мержить PR** | Влить ветку с тестами в целевую (обычно main/develop) | PR → Merge (после аппрува и зелёного CI) |

#### 🔄 Pull Request: что проверять тестировщику при код-ревью тестов

| Что проверяем | На что обратить внимание |
|---------------|--------------------------|
| **Ассерты** | Проверяют ли тесты правильные условия? Нет ли пустых тестов? |
| **Покрытие** | Есть ли тесты на позитивные и негативные сценарии? |
| **Независимость** | Тесты не зависят друг от друга (не полагаются на порядок выполнения) |
| **Очистка данных** | Тесты удаляют/откатывают тестовые данные после себя |
| **Читаемость** | Названия тестов понятны (`shouldReturn404WhenUserNotFound`) |
| **Wait-ы / таймауты** | Нет ли `Thread.sleep()`? Используются ли нормальные ожидания (WebDriverWait, Awaitility)? |
| **Уязвимости** | Нет хардкода паролей, токенов, секретов |
| **Пропущенные тесты** | Есть ли `@Ignore` / `@Disabled` без объяснения причины? |

#### 🧪 Интеграция с Jira

| Связка | Как работает |
|--------|--------------|
| **Smart commits** | В комментарии к коммиту можно указать Jira-задачу и её статус: `TEST-123 #comment tests added` |
| **Branch names** | Можно создать ветку прямо из задачи Jira (в интерфейсе) → ветка будет называться `TEST-123-fix-bug` |
| **Links в PR** | Если в PR упомянуть `TEST-123`, Jira отобразит ссылку на этот PR внутри задачи |
| **Build status on issue** | В Jira видно, для каких коммитов/веток пайплайн прошёл или упал |

#### 🚀 Bitbucket Pipelines (Cloud)

Bitbucket Pipelines — встроенный CI/CD, работает на основе YAML-файла `bitbucket-pipelines.yml` в корне репозитория.

**Пример для тестировщика (запуск тестов при каждом push):**

```yaml
image: maven:3.8-openjdk-17   # Docker-образ с Maven и JDK

pipelines:
  default:                      # Запускается при любом push в любую ветку
    - step:
        name: Run tests
        script:
          - mvn clean test      # Команда запуска тестов
        artifacts:              # Сохранить отчёты
          - target/surefire-reports/**
          
  pull-requests:
    '**':                        # Запускается для всех PR
      - step:
...
**Особенности Pipelines:**
- Бесплатно: до 50 минут в месяц (для небольшой команды)
- Платные тарифы: больше времени и параллельных билдов
- Поддерживает Docker, кэширование зависимостей, параллельные шаги
- Логи билдов доступны прямо в Bitbucket UI

#### 🔐 Аутентификация (как тестировщику подключиться)

| Способ | Команда | Особенности |
|--------|---------|-------------|
| **HTTPS + логин/пароль** | `git clone https://bitbucket.org/workspace/repo.git` | Запрашивает пароль каждый раз (неудобно) |
...
|---------|-----------|--------|--------|
| **Интеграция с Jira** | 🔥 Нативная (Atlassian) | ➕ Через плагин | ➕ Через плагин |
| **Private repos бесплатно** | ✅ (до 5 пользователей) | ✅ (неограниченно) | ✅ (неограниченно) |
| **Встроенный CI/CD** | Pipelines (Cloud) | GitHub Actions | GitLab CI (лучший) |
| **Code review** | Pull Requests | Pull Requests | Merge Requests |
| **Wiki** | ✅ (интеграция с Confluence) | ✅ (встроенная) | ✅ (встроенная) |
| **Популярность среди QA** | Команды, использующие Jira | Open Source, стартапы | Enterprise, DevOps-культура |

#### 🔗 Полезные ссылки

- [Официальный сайт](https://bitbucket.org/)
- [Документация](https://support.atlassian.com/bitbucket-cloud/)
- [Создание App Password](https://bitbucket.org/account/settings/app-passwords/)
- [Bitbucket Pipelines документация](https://support.atlassian.com/bitbucket-cloud/docs/configure-bitbucket-pipelinesyml/)
- [Миграция с Mercurial на Git](https://www.atlassian.com/git/tutorials/git-bisect)

[🔄 К содержанию - главы](#контроль-версий-и-ide-глава)  
[🔼 К содержанию](#content)
...
**IntelliJ IDEA** — это интегрированная среда разработки (IDE) от компании JetBrains, одна из самых популярных для Java, Kotlin и других JVM-языков. Широко используется тестировщиками для написания автотестов (Java + TestNG/JUnit), анализа кода, работы с Git и отладки тестов.

#### 📦 Версии

| Версия | Описание | Для кого |
|--------|----------|----------|
| **Community Edition (CE)** | Бесплатная, с открытым исходным кодом | Java, Kotlin, Maven, Gradle, Git |
| **Ultimate Edition (UE)** | Платная, пробная 30 дней | Всё из CE + веб-разработка, Spring, Docker, Kubernetes, БД, тестовые инструменты |

> 🧪 **Тестировщику**: для автоматизации на Java (Selenium, Rest Assured, JUnit) достаточно **Community Edition**. Ultimate нужна для работы с БД (встроенный Database Explorer), Docker, Kubernetes и Spring.

#### ⚙️ Основные возможности для тестировщика

| Возможность | Описание | Горячая клавиша |
|-------------|----------|-----------------|
| **Run тестов** | Запуск отдельных тестов, классов или всех тестов в пакете | `Ctrl+Shift+F10` (запуск текущего теста) |
| **Debug тестов** | Пошаговое выполнение, точки останова, просмотр переменных | `Shift+F9` (дебаг), `F8` (шаг с обходом), `F7` (шаг с заходом) |
| **Smart Completion** | Умное автодополнение кода с учётом контекста | `Ctrl+Space` |
| **Navigate to test** | Быстрый переход от класса к его тесту и обратно | `Ctrl+Shift+T` |
| **Search Everywhere** | Поиск по файлам, классам, методам, действиям | `Double Shift` |
| **Recent Files** | Список недавно открытых файлов | `Ctrl+E` |
| **Structure** | Просмотр структуры текущего файла (методы, поля) | `Alt+7` |
| **Find in Files** | Поиск текста по всем файлам проекта | `Ctrl+Shift+F` |
| **Replace in Files** | Замена текста по всем файлам проекта | `Ctrl+Shift+R` |
| **Git integration** | Встроенный GUI для git: commit, push, pull, diff, merge | `Ctrl+K` (commit), `Ctrl+Shift+K` (push) |
| **Terminal** | Встроенный терминал (cmd/bash) | `Alt+F12` |
| **Local History** | Локальная история изменений (даже без git) | ПКМ → Local History → Show History |

#### 🧪 Плагины для тестировщика

| Плагин | Назначение | Версия IDEA |
|--------|------------|-------------|
| **TestNG / JUnit** | Встроенные, поддержка фреймворков тестирования | CE / UE |
| **Cucumber for Java** | Поддержка BDD (Gherkin), навигация между .feature и step definitions | CE / UE |
| **Gherkin** | Подсветка синтаксиса .feature файлов | CE / UE |
| **RestAssured** | Автодополнение для REST API тестов | CE / UE |
| **SonarLint** | Статический анализ кода (баги, уязвимости, код-стайл) | CE / UE |
| **Save Actions** | Автоматическое форматирование при сохранении (import optimize, reformat) | CE / UE |
| **GitToolBox** | Расширенная информация о git (автор последнего изменения в строке) | CE / UE |
| **Indent Rainbow** | Раскраска отступов для читаемости | CE / UE |
| **Rainbow Brackets** | Раскраска скобок разных уровней | CE / UE |
| **Database Tools** | Работа с БД (SQL editor, просмотр таблиц) — **только Ultimate** | UE |

#### 🐞 Работа с дебагом тестов

| Действие | Горячая клавиша | Что делает |
|----------|----------------|------------|
| Точка останова (breakpoint) | `Ctrl+F8` (на строке) | Останавливает выполнение на выбранной строке |
| Conditional breakpoint | ПКМ на breakpoint → Condition | Останавливается только при выполнении условия (например, `i==5`) |
| Start Debug | `Shift+F9` | Запускает текущий тест в режиме отладки |
| Step Over | `F8` | Выполнить текущую строку и перейти к следующей |
| Step Into | `F7` | Зайти внутрь метода на текущей строке |
| Step Out | `Shift+F8` | Выйти из текущего метода и вернуться на уровень выше |
| Resume Program | `F9` | Продолжить выполнение до следующей точки останова |
| Evaluate Expression | `Alt+F8` | Выполнить любой код / выражение в текущем контексте (проверить значение) |
| Stop Debug | `Ctrl+F2` | Остановить сессию отладки |

#### 🔧 Полезные настройки для тестировщика

| Настройка | Где искать | Зачем |
|-----------|------------|-------|
| **Auto-import** | Settings → Editor → General → Auto Import | Автоматически добавляет импорты и удаляет неиспользуемые |
| **Show line numbers** | Settings → Editor → General → Appearance → Show line numbers | Номера строк для навигации |
| **Show method separators** | Settings → Editor → General → Appearance → Show method separators | Визуальное разделение методов |
| **Change font size with Ctrl+Mouse Wheel** | Settings → Editor → General → Change font size with Ctrl+Mouse Wheel | Удобное масштабирование |
| **Add unambiguous imports on the fly** | Settings → Editor → General → Auto Import → Add unambiguous imports on the fly | Не спрашивает, если импорт однозначный |
| **Maven / Gradle auto-reload** | Settings → Build Tools → Maven → Reload project after changes | Автоматически перезагружает зависимости |
| **Test runner (по умолчанию)** | Settings → Build Tools → Gradle → Run tests using → IntelliJ IDEA | Быстрее запускает тесты, чем Gradle |

#### 🚀 Лайфхаки для тестов

| Лайфхак | Как сделать |
|---------|-------------|
| **Запустить только один тест** | Кликнуть на зелёную стрелку слева от `@Test` или рядом с именем метода |
| **Перезапустить провалившийся тест** | `Ctrl+Shift+F10` после провала — IDEA предложит перезапустить только упавшие |
| **Сравнить ожидаемый и фактический результат** | В дебаге выделить оба → `Ctrl+D` (Show Diff) |
| **Сгенерировать тестовый класс** | Внутри тестируемого класса → `Alt+Insert` → Test → выбрать методы и фреймворк |
| **Быстрый запуск последнего теста** | `Shift+F10` (без дебага), `Shift+F9` (с дебагом) |
| **Закомментировать блок кода** | Выделить → `Ctrl+Shift+/` |
| **Раскомментировать блок кода** | Выделить → `Ctrl+Shift+/` |
| **Дублировать строку** | `Ctrl+D` |
| **Удалить строку** | `Ctrl+Y` |
| **Переместить строку вверх/вниз** | `Shift+Alt+Up` / `Shift+Alt+Down` |

#### 🐳 Интеграция с Docker (Ultimate)

- Просмотр контейнеров, образов, логов прямо из IDE
- Запуск / остановка / удаление контейнеров
- Подключение к терминалу контейнера
- Docker Compose поддержка с визуализацией сервисов

#### 🔗 Полезные ссылки

- [Официальный сайт](https://www.jetbrains.com/idea/)
- [Скачать Community Edition](https://www.jetbrains.com/idea/download/)
- [Шпаргалка по горячим клавишам (Windows/Linux)](https://resources.jetbrains.com/storage/products/intellij-idea/docs/IntelliJIDEA_ReferenceCard.pdf)
- [Документация по дебагу](https://www.jetbrains.com/help/idea/debugging-code.html)
- [Плагины Marketplace](https://plugins.jetbrains.com/)
...
**Apache Kafka** — это распределённая потоковая платформа с открытым исходным кодом для построения высоконагруженных конвейеров событий. Используется как брокер сообщений, но в отличие от RabbitMQ ориентирована на высокую пропускную способность и долговременное хранение данных.

#### 📐 Основные понятия

| Понятие | Описание |
|---------|----------|
| **Producer** | Отправитель, который публикует сообщения в топики |
| **Consumer** | Получатель, который читает сообщения из топиков |
| **Consumer Group** | Группа потребителей, которые совместно читают топик (каждый получает свою партицию) |
| **Topic** | Логический канал (категория), куда отправляются сообщения |
| **Partition** | Тема разделена на партиции — единицы параллелизма и порядка |
| **Offset** | Уникальный идентификатор сообщения внутри партиции (счётчик) |
| **Broker** | Узел Kafka (сервер), хранит партиции и обслуживает запросы |
| **ZooKeeper** | Координация брокеров (управление кластером) — **устаревает**, используется KRaft |
| **Leader / Follower** | Лидер партиции обслуживает чтение/запись, фолловеры реплицируют данные |
| **Replication Factor** | Количество копий партиции для отказоустойчивости |
| **Retention Policy** | Политика хранения сообщений (по времени или по размеру) |
...
| Команда | Описание |
|---------|----------|
| `kafka-topics --bootstrap-server localhost:9092 --list` | Список всех топиков |
| `kafka-topics --bootstrap-server localhost:9092 --create --topic my_topic --partitions 3 --replication-factor 1` | Создать топик с 3 партициями |
| `kafka-topics --bootstrap-server localhost:9092 --describe --topic my_topic` | Детальная информация о топике |
| `kafka-topics --bootstrap-server localhost:9092 --delete --topic my_topic` | Удалить топик |
| `kafka-console-producer --bootstrap-server localhost:9092 --topic my_topic` | Отправить сообщение вручную (консоль) |
| `kafka-console-consumer --bootstrap-server localhost:9092 --topic my_topic --from-beginning` | Прочитать все сообщения с начала |
| `kafka-console-consumer --bootstrap-server localhost:9092 --topic my_topic --group my_group` | Читать как часть consumer group |
| `kafka-consumer-groups --bootstrap-server localhost:9092 --list` | Список всех consumer groups |
| `kafka-consumer-groups --bootstrap-server localhost:9092 --describe --group my_group` | Статус группы (lag, offset) |
| `kafka-run-class kafka.tools.GetOffsetShell --bootstrap-server localhost:9092 --topic my_topic --time -1` | Посмотреть последние offset'ы |
| `kafka-configs --bootstrap-server localhost:9092 --alter --entity-type topics --entity-name my_topic --add-config retention.ms=86400000` | Установить retention 24 часа |

#### 📊 Ключевые метрики для тестировщика

| Метрика | Что показывает | Как проверить |
|---------|----------------|---------------|
| **Consumer Lag** | Отставание consumer'а от producer'а (сообщения накопились, но не прочитаны) | `kafka-consumer-groups --describe` |
| **Messages in / out** | Скорость записи и чтения | Веб-интерфейсы (AKHQ, Confluent Control Center) |
| **Under-replicated partitions** | Партиции, у которых не хватает реплик | `kafka-topics --describe --under-replicated-partitions` |
| **Offline partitions** | Партиции без лидера | `kafka-topics --describe --unavailable-partitions` |
| **Segment size / retention** | Как долго хранятся сообщения | `kafka-log-dirs --describe` |

#### 🧪 Что проверять тестировщику при работе с Kafka

| Что проверяем | Как проверяем | Почему важно |
|---------------|---------------|---------------|
| **Отправка сообщения** | Producer отправил → offset увеличился → сообщение можно прочитать | Базовая работоспособность |
| **Consumer Lag = 0** | Все сообщения прочитаны, нет отставания | Приложение успевает обрабатывать поток |
| **Порядок сообщений** | Сообщения с одним key попадают в одну партицию → порядок гарантирован | Бизнес-логика (агрегаты, события) |
| **Idempotency** | Повторная отправка сообщения не дублируется (producer idempotent) | Избежать дублей при сбоях сети |
| **Exactly-once semantics** | Каждое сообщение обработано ровно один раз (транзакции, idempotent producer + consumer) | Финансовые и критичные сценарии |
| **Retention policy** | Сообщения старше retention.ms исчезают / компактятся | Экономия места, соответствие требованиям |
| **Rebalancing** | Consumer упал → его партиции перераспределились между живыми | Отказоустойчивость |
| **Dead Letter Queue (DLQ)** | Необрабатываемые сообщения → отправляются в DLQ топик для анализа | Не терять проблемные данные |
| **Key-based routing** | Сообщения с одинаковым key всегда в одной партиции | Локальная обработка связанных событий |

#### 🐳 Пример тестового окружения через Docker Compose
...
services:
  ...
  # Опционально: веб-интерфейс для Kafka
  akhq:
...
- **AKHQ** веб-интерфейс: `http://localhost:8080` (можно смотреть топики, партиции, consumer groups, lag)

#### 🔄 Гарантии доставки

| Гарантия | Описание | Настройка |
|----------|----------|-----------|
| **At most once** | Сообщение может потеряться, но не обработается дважды | `acks=0` |
| **At least once** | Сообщение точно доставлено, но может дублироваться | `acks=1` или `all` + consumer без идемпотентности |
| **Exactly once** | Сообщение доставлено ровно один раз | `acks=all` + `enable.idempotence=true` + транзакционный consumer |

#### 🔗 Полезные ссылки

- [Официальный сайт](https://kafka.apache.org/)
- [Документация](https://kafka.apache.org/documentation/)
- [AKHQ — веб-интерфейс для Kafka](https://github.com/tchiotludo/akhq)
- [Confluent Platform (Kafka + удобные тулзы)](https://www.confluent.io/product/confluent-platform/)

[🔄 К содержанию - главы](#брокеры-сообщений-глава)  
[🔼 К содержанию](#content)
...
**RabbitMQ** — это популярный брокер сообщений, реализующий протоколы AMQP 0-9-1, MQTT, STOMP. Он используется для асинхронного взаимодействия между сервисами, обработки фоновых задач и построения событийно-ориентированных архитектур.
...
| **Queue** | Очередь, где хранятся сообщения (FIFO) |
| **Exchange** | Маршрутизатор, который направляет сообщения в очереди |
| **Binding** | Связь между exchange и очередью с указанием routing key |
| **Routing key** | Ключ, по которому exchange маршрутизирует сообщение |
| **Virtual Host** | Изолированная среда для разделения ресурсов между приложениями |

#### 🔄 Типы обменников (Exchanges)

| Тип | Описание | Пример использования |
|-----|----------|----------------------|
| **Direct** | Сообщение отправляется в очередь с точно совпадающим routing key | Логи по уровням (info, warning, error) |
| **Topic** | Маршрутизация по шаблону (wildcard: `*` - одно слово, `#` - ноль или более) | Логи от разных сервисов: `user.created`, `order.paid` |
| **Fanout** | Рассылает сообщение во все привязанные очереди (игнорирует routing key) | Широковещательные уведомления |
| **Headers** | Маршрутизация по заголовкам сообщения (вместо routing key) | Сложные правила фильтрации |

#### 🔧 Основные команды CLI (rabbitmqctl)

| Команда | Описание |
|---------|----------|
| `sudo rabbitmqctl status` | Статус ноды и кластера |
| `sudo rabbitmqctl list_queues` | Список всех очередей и количества сообщений |
| `sudo rabbitmqctl list_queues name messages_ready messages_unacknowledged` | Детальная информация об очередях |
| `sudo rabbitmqctl list_exchanges` | Список обменников |
| `sudo rabbitmqctl list_bindings` | Список всех привязок |
| `sudo rabbitmqctl purge_queue <queue_name>` | Очистить очередь (удалить все сообщения) |
| `sudo rabbitmqctl delete_queue <queue_name>` | Удалить очередь |
| `sudo rabbitmqctl add_user <username> <password>` | Создать пользователя |
| `sudo rabbitmqctl set_permissions -p <vhost> <user> ".*" ".*" ".*"` | Назначить права |
| `sudo rabbitmqctl set_user_tags <user> administrator` | Назначить роль администратора |

#### 🖥️ Веб-интерфейс (Management Plugin)

- **URL**: `http://localhost:15672`
- **Логин по умолчанию**: `guest` / `guest` (только с localhost)
- **Что можно делать**:
  - Просматривать очереди, обменники, привязки
  - Создавать/удалять очереди и обменники
  - Публиковать сообщения вручную
  - Просматривать и удалять сообщения из очередей
  - Мониторить скорость (messages in/out)
  - Смотреть графики нагрузки и потребления памяти

#### 🧪 Что проверять тестировщику при работе с RabbitMQ

| Что проверяем | Как проверяем |
|---------------|---------------|
| **Отправка сообщения** | Продюсер отправил → сообщение появилось в очереди (веб-интерфейс / `rabbitmqctl list_queues`) |
| **Получение сообщения** | Консьюмер забрал → сообщение исчезло из очереди (очередь пустая или messages_ready = 0) |
| **Dead Letter Queue (DLQ)** | Сообщение не обработано (ошибка, ttl истек) → попало в DLQ → проверить наличие |
| **Порядок сообщений** | При использовании одного consumer и одной очереди порядок сохраняется |
| **Retry / Requeue** | При ошибке сообщение возвращается в очередь (проверить, что оно снова появляется) |
| **Idempotency** | При повторной обработке одного сообщения нет дублей в БД или побочных эффектах |
| **TTL (Time To Live)** | Сообщение живет заданное время → удаляется или уходит в DLQ |
| **Max Length** | При превышении лимита: удаляются старые или все (зависит от политики overflow) |
| **Cluster / HA** | Один узел упал → очередь перешла на другой узел, сообщения не потеряны |
...
После запуска `docker-compose up -d` — открываешь `localhost:15672`, логин `test_user` / `test_pass`, vhost `test_vhost`.

#### 🔗 Полезные ссылки

- [Официальный сайт](https://www.rabbitmq.com/)
- [Документация](https://www.rabbitmq.com/docs)
