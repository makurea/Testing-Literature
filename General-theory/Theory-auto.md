...
|--------------------------|-----------------------------------------------------------|----------------------------------------------------------------------------|
| **Действия**             | `setValue(String text)`                                   | Очищает поле и вводит текст                                                |
| **Действия**             | `val(String text)`                                        | Синоним `setValue`                                                         |
| **Действия**             | `append(String text)`                                     | Добавляет текст в конец текущего значения поля                             |
| **Действия**             | `pressEnter()`                                            | Нажимает клавишу Enter в поле                                              |
| **Действия**             | `pressTab()`                                              | Нажимает клавишу Tab                                                       |
| **Действия**             | `click()`                                                 | Одиночный клик по элементу                                                 |
| **Действия**             | `doubleClick()`                                           | Двойной клик                                                               |
| **Действия**             | `contextClick()`                                          | Клик правой кнопкой мыши (вызов контекстного меню)                         |
| **Действия**             | `hover()`                                                 | Наводит курсор на элемент (без клика)                                      |
| **Действия**             | `dragAndDropTo(String targetSelector)`                    | Перетаскивает элемент к другому элементу (по селектору)                    |
| **Действия**             | `dragAndDropTo(WebElement target)`                        | Перетаскивает элемент к указанному объекту                                 |
| **Действия**             | `scrollTo()`                                              | Прокручивает страницу так, чтобы элемент стал видимым                      |
| **Действия**             | `scrollIntoView(boolean alignToTop)`                      | Прокручивает с дополнительным выравниванием (true – к верху)              |
| **Действия**             | `selectOption(String value)`                              | Выбирает опцию в выпадающем списке по видимому тексту                      |
| **Действия**             | `selectOptionByValue(String value)`                       | Выбирает опцию по значению атрибута `value`                                |
| **Действия**             | `selectRadio(String value)`                               | Выбирает радиокнопку с указанным значением                                 |
| **Действия**             | `setSelected(boolean selected)`                           | Устанавливает состояние чекбокса/радиокнопки (true – включить)            |
| **Действия**             | `uploadFile(File file)`                                   | Загружает файл в поле типа `file`                                          |
| **Действия**             | `uploadFile(String filePath)`                             | Загружает файл по указанному пути                                          |
| **Проверки и ожидания**  | `shouldBe(Condition condition)`                           | Проверяет, что элемент соответствует условию (например, `visible`, `enabled`, `focused`, `hidden`) |
| **Проверки и ожидания**  | `shouldHave(Condition condition)`                         | Проверяет, что элемент имеет свойство (например, `text("...")`, `attribute("class", "active")`, `value("...")`) |
| **Проверки и ожидания**  | `shouldNotBe(Condition condition)`                        | Проверяет, что элемент **не** соответствует условию                        |
| **Проверки и ожидания**  | `shouldNotHave(Condition condition)`                      | Проверяет, что элемент **не** имеет свойство                               |
| **Проверки и ожидания**  | `waitUntil(Condition condition, long timeout)`            | Ожидает, пока элемент не станет соответствовать условию (с указанием таймаута) |
| **Получение данных**     | `getText()`                                               | Возвращает видимый текст элемента                                          |
| **Получение данных**     | `getAttribute(String name)`                               | Возвращает значение атрибута (например, `"href"`, `"class"`)               |
| **Получение данных**     | `getValue()`                                              | Возвращает значение поля (аналог `getAttribute("value")`)                  |
| **Получение данных**     | `exists()`                                                | Проверяет, присутствует ли элемент в DOM (без учёта видимости)            |
| **Получение данных**     | `isDisplayed()`                                           | Проверяет, виден ли элемент в данный момент (лучше использовать `shouldBe`) |
| **Получение данных**     | `isEnabled()`                                             | Проверяет, доступен ли элемент для взаимодействия                          |
| **Получение данных**     | `innerText()`                                             | Возвращает текст вместе с дочерними элементами (включая скрытый)          |
| **Получение данных**     | `data(String key)`                                        | Возвращает значение атрибута `data-*` (например, `data("id")`)            |
| **Браузер/окружение**    | `switchToFrame(WebElement frame)`                         | Переключает контекст в указанный фрейм                                     |
| **Браузер/окружение**    | `switchToDefaultContent()`                                | Возвращается в основной контент страницы (выход из фрейма)                |
| **Браузер/окружение**    | `alert()`                                                 | Возвращает объект `Alert` для работы с всплывающими окнами (`accept()`, `dismiss()`, `getText()`) |
| **Браузер/окружение**    | `screenshot()`                                            | Делает скриншот текущего состояния элемента (возвращает `File`)           |
