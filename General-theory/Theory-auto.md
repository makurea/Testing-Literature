...
**Mockito** — это популярный фреймворк для **мокирования** (создания имитаций) в юнит-тестах для Java. Он позволяет создавать фиктивные (mock) объекты и объекты-заглушки (stub), чтобы изолировать тестируемый код от его реальных зависимостей (например, от базы данных, внешних API или сложных сервисов). Это обеспечивает быстрые и надёжные юнит-тесты.

| Аспект | Описание | Зачем это нужно QA-автоматизатору |
| :--- | :--- | :--- |
| **Основная задача** | Изоляция тестируемого класса. Mockito позволяет **имитировать** поведение зависимостей. | Ускоряет юнит-тесты и делает их **детерминированными**. Вы можете симулировать любой сценарий (успех, ошибку, исключение), не обращаясь к реальным внешним ресурсам. |
| **Мокирование (Mocking)** | Создание поддельного объекта, который **записывает**, какие методы были вызваны. | Проверка **взаимодействия** (например, убедиться, что метод `saveUser()` был вызван один раз). |
| **Заглушка (Stubbing)** | Настройка поддельного объекта на **возврат конкретного значения** при вызове определённого метода. | Обеспечение **контролируемых входных данных** (например, сказать сервису, что при запросе `getUser(1)` он должен вернуть заранее определённый объект). |
| **Ключевые аннотации** | `@Mock`, `@InjectMocks`, `@Spy`. | Упрощают инициализацию мок-объектов и их внедрение в тестируемый класс. |
