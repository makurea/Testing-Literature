...
**WireMock** — это гибкий инструмент, который выступает в роли **HTTP-мок-сервера** (Mock Server). Он позволяет создавать **заглушки (stubs)** для внешних API и веб-сервисов, чтобы имитировать их поведение. Это критически важно для **изоляции** ваших тестов от реальных, медленных или нестабильных внешних зависимостей.

#### Назначение и Возможности

| Аспект | Описание | Зачем это нужно QA-автоматизатору |
| :--- | :--- | :--- |
| **Основная задача** | Мокирование (создание имитации) реальных внешних HTTP-сервисов. | Позволяет тестировать ваш код (API-клиенты, микросервисы) в **изоляции** и **контролируемой среде**. Вы можете симулировать любые сценарии. |
| **Создание заглушек (Stubbing)** | Позволяет настроить сервер так, чтобы при получении конкретного запроса (по URL, методу, заголовкам) он возвращал заранее заданный ответ. | Вы можете имитировать **успешные ответы** (200), **ошибки** (404, 500) или **невалидные данные** для проверки обработки ошибок. |
| **Проверка взаимодействия** | WireMock записывает все входящие запросы. | Позволяет **проверить**, что ваш тестируемый код отправил правильный запрос мок-серверу (например, корректные заголовки или тело JSON). |
| **Имитация задержек** | Возможность добавлять искусственные задержки в ответы. | Тестирование таймаутов, медленного соединения или поведения приложения в условиях **высокой задержки** (latency testing). |
| **Режимы работы** | Может запускаться как **библиотека** (внутри JVM-теста), как **отдельный процесс** или как **Docker-контейнер**. | Гибкость для использования в юнит-тестах, интеграционных тестах или в CI/CD-пайплайне. |
