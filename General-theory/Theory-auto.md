...
| Метод                                                     | Назначение                                                                 |
|-----------------------------------------------------------|----------------------------------------------------------------------------|
| `setValue(String text)`                                   | Очищает поле и вводит текст                                                |
| `val(String text)`                                        | Синоним `setValue`                                                         |
| `append(String text)`                                     | Добавляет текст в конец текущего значения поля                             |
| `pressEnter()`                                            | Нажимает клавишу Enter в поле                                              |
| `pressTab()`                                              | Нажимает клавишу Tab                                                       |
| `click()`                                                 | Одиночный клик по элементу                                                 |
| `doubleClick()`                                           | Двойной клик                                                               |
| `contextClick()`                                          | Клик правой кнопкой мыши (вызов контекстного меню)                         |
| `hover()`                                                 | Наводит курсор на элемент (без клика)                                      |
| `dragAndDropTo(String targetSelector)`                    | Перетаскивает элемент к другому элементу (по селектору)                    |
| `dragAndDropTo(WebElement target)`                        | Перетаскивает элемент к указанному объекту                                 |
| `scrollTo()`                                              | Прокручивает страницу так, чтобы элемент стал видимым                      |
| `scrollIntoView(boolean alignToTop)`                      | Прокручивает с дополнительным выравниванием (true – к верху)              |
| `selectOption(String value)`                              | Выбирает опцию в выпадающем списке по видимому тексту                      |
| `selectOptionByValue(String value)`                       | Выбирает опцию по значению атрибута `value`                                |
| `selectRadio(String value)`                               | Выбирает радиокнопку с указанным значением                                 |
| `setSelected(boolean selected)`                           | Устанавливает состояние чекбокса/радиокнопки (true – включить)            |
| `uploadFile(File file)`                                   | Загружает файл в поле типа `file`                                          |
| `uploadFile(String filePath)`                             | Загружает файл по указанному пути                                          |
| `shouldBe(Condition condition)`                           | Проверяет, что элемент соответствует условию (например, `visible`, `enabled`, `focused`, `hidden`) |
| `shouldHave(Condition condition)`                         | Проверяет, что элемент имеет свойство (например, `text("...")`, `attribute("class", "active")`, `value("...")`) |
| `shouldNotBe(Condition condition)`                        | Проверяет, что элемент **не** соответствует условию                        |
| `shouldNotHave(Condition condition)`                      | Проверяет, что элемент **не** имеет свойство                               |
| `waitUntil(Condition condition, long timeout)`            | Ожидает, пока элемент не станет соответствовать условию (с указанием таймаута) |
| `getText()`                                               | Возвращает видимый текст элемента                                          |
| `getAttribute(String name)`                               | Возвращает значение атрибута (например, `"href"`, `"class"`)               |
| `getValue()`                                              | Возвращает значение поля (аналог `getAttribute("value")`)                  |
| `exists()`                                                | Проверяет, присутствует ли элемент в DOM (без учёта видимости)            |
| `isDisplayed()`                                           | Проверяет, виден ли элемент в данный момент (лучше использовать `shouldBe`) |
| `isEnabled()`                                             | Проверяет, доступен ли элемент для взаимодействия                          |
| `switchToFrame(WebElement frame)`                         | Переключает контекст в указанный фрейм                                     |
| `switchToDefaultContent()`                                | Возвращается в основной контент страницы (выход из фрейма)                |
| `alert()`                                                 | Возвращает объект `Alert` для работы с всплывающими окнами (`accept()`, `dismiss()`, `getText()`) |
| `screenshot()`                                            | Делает скриншот текущего состояния элемента (возвращает `File`)           |
| `innerText()`                                             | Возвращает текст вместе с дочерними элементами (включая скрытый)          |
| `data(String key)`                                        | Возвращает значение атрибута `data-*` (например, `data("id")`)            |
